...
function out2console() {
    ...
    console.log("firstletter is " + revval[0])
};
...
function fibonacci() {
    var prevTotal = 0,
        prevAdder = 1,
        fibUpto = document.getElementById("fibupto").value,
        fibSeries = [],
        currTotal = 0;

    for (i = 0; i <= fibUpto; i++) {
        currTotal = prevTotal + prevAdder;
        fibSeries.push(currTotal);
        prevAdder = currTotal - prevAdder;
        prevTotal = currTotal;
    }
    document.getElementById("fibresult").innerHTML = fibSeries;
    document.getElementById("fibupto").value = "";
};